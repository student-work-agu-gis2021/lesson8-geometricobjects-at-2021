...
def create_line_geom(points):
  ...
  for n in points:
    assert type(n)==Point,"All list values should be Shapely Point objects!"
...
def create_poly_geom(coords):
  ...
  for n in coords:
    assert type(coords[n])==tuple,"All list values should be coordinate tuples!"
  poly=Polygon(coords)
  return poly
...
def get_area(poly):
  assert type(poly)==Polygon,"Input should be a Shapely Polygon -object!"
  return poly.area
